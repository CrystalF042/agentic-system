...
from cio import cro_inputs, regime, risk_officer, runid, sizing    # noqa: E402
...
def _measures_for(symbol: str) -> dict:
    """二部口径的确定性测量。**实现在 `cio.cro_inputs`，这里只是个别名。**

    build122 把它搬走了：自动流水线（`research/pipeline.py`）也要跑 CRO，
    两个入口必须用同一份换算。抄一份到那边的下场是——手动这份被用例覆盖着、
    自动那份没有，改了其中一份另一份继续用旧口径，**而两边都不报错。**
    """
    return cro_inputs.measures_for(symbol)
